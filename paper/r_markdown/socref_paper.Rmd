...
```{r}
load(path(project_root, "paper", "processed_data", "soc_ref_e2.RData"))

load(path(project_root, "paper", "processed_data", "soc_ref_e1.RData"))

#creates data frame with mean age in months for all four age bins
age_months <- 
  d %>%
  group_by(age_years) %>%
  summarise(mean = mean(age_months)) %>% 
  mutate(
    age_years = factor( #turn age into a factor here
      age_years,
      levels = c(2, 3, 4, 5),
...
# age_months$age_years <- factor(age_months$age_years, levels = c(2, 3, 4, 5), labels = c("two", "three", "four", "five"))

#makes an entirely new data frame with a column indicating 
#if the trial number is 1 through 4
halfs <- 
  d %>%
  mutate(first_half = between(trial, 1, 4)) 

#gives a list of summary statistics about each phase but I am confused by this code?
plength1 <- psych::describeBy(d$phase_length, d$phase_name)
plength2 <- psych::describeBy(d$phase_length, d$phase_name)

#initialize 2 data frames for the phases
phases1 <- data.frame()
phases2 <- data.frame()

#create data frames with mean and sd for all four phases in both experiments
phases1 <- 
  bind_rows(
    phases1, 
    plength1$label, 
...
#combine both phase data frames
phases <- 
  bind_rows(phases1, phases2) %>%
  select(Experiment, phase, mean, sd)

save(
  phases, 
  file = path(project_root, "paper", "processed_data", "phases.RData")
)
```
...
```{r echo = FALSE}
#This chunk calculates a bunch of values that Emily uses later in the paper.

#how often did children select two objects or no objects?

#data frame of distinct SID's
ids <- 
  d %>% 
  distinct(SID)

#data frame of distinct trials
trials <- 
  d %>% 
  distinct(trial)

#multiply number of trials by number of subjects to get 960
...
two_kids <- unique(two_objs$SID) #number of kids who picked up two items
...
#generate data frame for 2-object familiar trials
cor <- 
  d %>%
...
#get summary stats for accuracy on 2-obj familiar trials
acc_sum <- 
  cor %>%
  filter(acc == 1) %>%
  group_by(age_years) %>%
...
#load phase data
load(path(project_root, "paper", "processed_data", "phases.RData"))

#name columns in phases data frame
...
```{r}
rm(list = ls()) #clear the environment for some reason?
...
# 
# 
# #first group by SID, phase, number of objects, and familiarity
# msslooks <- 
#   filter(d, exclude == 0 | response != "LR" | response != "RL") %>%
#   group_by(SID, phase_name, num_objs, familiarity, age_years) %>%
#   summarise(num_looks = mean(num_looks))
# 
# 
# #then bootstrap to generate confidence intervals
# mslooks <- 
#   filter(d, exclude == 0 | response != "LR" | response != "RL") %>%
...
# # msslooks <- 
# #   msslooks %>% 
# #   mutate(phase_name = factor(
# #     phase_name,
# #     levels = c("label", "slide", "planning", "resonse")
# #   ))
# 
# #making some columns into factors instead of character values
# msslooks$phase_name <- 
#   factor(
#     msslooks$phase_name, 
...
```{r}
#trying the data manipulation in the last chunk in a cleaner way

#this creates a data frame that has subject-level means for each familiarity-num_objs combo.
msslooks <- 
  d %>% 
  filter(exclude == 0 | response != "LR" | response != "RL") %>%
  group_by(SID, phase_name, num_objs, familiarity, age_years) %>%
  summarise(num_looks = mean(num_looks)) %>% 
  ungroup %>% 
  mutate(
...
#creates a data frame that provides summary statistics across
#participants for each familiarity-num_objs combo
mslooks <- 
  filter(d, exclude == 0 | response != "LR" | response != "RL") %>%
  group_by(phase_name, familiarity, num_objs, age_years) %>%
  tidyboot_mean(column = num_looks)  %>% 
...
```{r include = FALSE}
#prepare data frame for the linear mixed effect model
lmer_data <- 
  d %>%
  filter(!exclude) %>%
  mutate(
    num_objs = factor(num_objs), 
...
save(e1.tab, file = path(project_root, "paper", "processed_data", "e1tab.RData"))
```
...
missingthree <- missing$excluded[missing$age_years == 3] #didn't she already define these?
missingfour <- missing$excluded[missing$age_years == 4]  #why is she doing this again?
...
age_months$age_years <- 
  factor(age_months$age_years, levels = c(3, 4), labels = c("three", "four"))
```
...
nov_t <- 
  ...
  mutate(
    both = response == "LR" | response == "RL" | response == "B", 
    unambiguous = trial_type == "mutual" | 
      trial_type == "familiar" | Gaze == "gaze"
  ) %>%
